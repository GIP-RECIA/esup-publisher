...
public class ContentDTOFactoryImpl implements ContentDTOFactory {
	...
	private ContentDTO from(@NotNull AbstractItem model) {
		...
		Set<Subscriber> subscribers = Sets.newHashSet(subscriberRepository.findAll(SubscriberPredicates.onCtx(model
				.getContextKey())));
		List<SubscriberFormDTO> targets = Lists.newArrayList();
		if (!subscribers.isEmpty()) {
			for (Subscriber subscriber : subscribers) {
				if (subscriber.getSubjectCtxId().getSubject().getKeyType().getId() == SubjectType.GROUP.getId()
						|| subscriber.getSubjectCtxId().getSubject().getKeyType().getId() == SubjectType.PERSON.getId()) {
					targets.add(new SubscriberFormDTO(subjectDTOSimpleFactory.from(new SubjectKey(subscriber
							.getSubjectCtxId().getSubject().getKeyValue(), subscriber.getSubjectCtxId().getSubject()
							.getKeyType())), subscriber.getSubscribeType()));
				} else {
					targets.add(new SubscriberFormDTO(subjectKeyExtendedDTOFactory.convertToDTOKey(subscriber
							.getSubjectCtxId().getSubject()), subscriber.getSubscribeType()));
				}
			}
		}
		...
		if (!files.isEmpty()) {
			for (LinkedFileItem file : files) {
				pathFiles.add(new LinkedFileItemDTO(file.getUri(), file.getFilename(), file.isInBody(), file
						.getContentType()));
			}
		}
