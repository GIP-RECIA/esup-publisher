  ...
  <div class="container-fluid justify-content-center">
    ...
    <div id="navbar-collapse" class="collapse navbar-collapse">
      <ul class="nav navbar-nav nav-pills justify-content-center">
          ...
          <ul class="dropdown-menu">
            <li :class="{ active: isPageNameIncludes('owned') }">
              <router-link to="/contents/owned/DRAFT" class="dropdown-item">
                <span class="fas fa-asterisk" /> &#xA0;<span>{{ $t('global.menu.manager.contents.owned') }}</span>
              </router-link>
            </li>
            <li :class="{ active: isPageNameIncludes('managed') }">
              <router-link to="/contents/managed/PENDING" class="dropdown-item">
                <span class="fas fa-asterisk" /> &#xA0;<span>{{ $t('global.menu.manager.contents.managed') }}</span>
              </router-link>
            </li>
          </ul>
        </li>
        ...
        <li v-if="canModerate" :class="{ active: isPageNameIncludes('treeview') }">
          <router-link to="/treeview" class="nav-link">
            <span class="fas fa-wrench fa-2x" />
