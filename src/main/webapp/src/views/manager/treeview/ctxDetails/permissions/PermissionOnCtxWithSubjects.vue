        ...
        <ul class="list-group list-unstyled">
          <li v-for="target in permission.authorizedSubjects" :key="target.keyId" class="list-group-item">
            <esup-subject-infos .subject="target" .config="subjectInfosConfig" .onSubjectClicked="() => viewSubject(subject)">
              <a v-tooltip="$t('manager.publish.targets.remove')" href="" @click.prevent="removePermTarget(target)"><i class="far fa-trash-can text-danger" /></a>&nbsp;
            </esup-subject-infos>
          </li>
        </ul>
        <div v-if="!permission.authorizedSubjects || permission.authorizedSubjects.length < 1" class="invalid-feedback d-block">
          {{ $t('entity.validation.required') }}
        </div>
      </div>
