...
export default {
  ...
  provide() {
    return {
      ...
      setCtxPermissionType: (ctxPermissionType) => {
        this.ctxPermissionType = ctxPermissionType
      },
  ...
  methods: {
    ...
    loadAvailableTypes(isEditCurrentCtx) {
      const ctx = isEditCurrentCtx ? this.editedContext : this.context
    ...
    loadAvailableRoles(withRole) {
      ...
      if (!CommonUtils.equals(this.ctxType, 'ORGANIZATION')) {
        this.availableRoles = this.availableRoles.filter((element) => {
  ...
  <div class="ctxDetails">
    <ul class="nav nav-pills">
      <li class="nav-item">
        <a
          class="nav-link"
          :aria-current="activeNav === 'content' || !activeNav"
          :class="{ active: activeNav === 'content' || !activeNav }"
          href=""
      ...
      <li v-if="hasPermissionManagment && canManage" class="nav-item">
        <a
          class="nav-link"
          :aria-current="activeNav === 'permissions'"
      ...
      <li v-if="hasTargetManagment && canManage" class="nav-item">
        <a
          class="nav-link"
          :aria-current="activeNav === 'targets'"
