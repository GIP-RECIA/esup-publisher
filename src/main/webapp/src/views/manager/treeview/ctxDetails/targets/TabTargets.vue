...
<template>
  <div>
    <h3 v-if="context.contextKey.keyType !== 'ORGANIZATION' && context.contextKey.keyType !== 'PUBLISHER'" class="mt-3 mb-2">
    ...
    <div v-show="addSubs" aria-labelledby="addSubscriberOnContext" aria-hidden="true">
      <form name="editSubscriberForm" role="form" novalidate>
        ...
        <div class="body">
          <div v-if="showSubscribeType()" class="form-group">
            <label class="control-label" for="subscribeType">{{ $t('subscriber.subscribeType') }}</label>
            <select id="subscribeType" v-model="subscriber.subscribeType" class="form-select" name="subscribeType">
              <option v-for="subscribeType in subscribeTypeList" :key="subscribeType.id" :value="subscribeType.name">
                {{ $t(subscribeType.label) }}
              </option>
            </select>
            <div v-if="!subscriber.subscribeType || subscriber.subscribeType === {}" class="invalid-feedback d-block">
          ...
          <div class="form-group">
            ...
            <ul v-if="isSubjectSelected()" class="list-group list-unstyled">
              <li class="list-group-item">
                ...
                >
                  <a v-tooltip="$t('manager.publish.targets.remove')" href="" @click.prevent="removeSelectedSubject()"><i class="far fa-trash-can text-danger" /></a>&nbsp;
                </esup-subject-infos>
              </li>
            </ul>
            <div v-if="!isSubjectSelected()" class="invalid-feedback d-block">
              {{ $t('entity.validation.required') }}
            </div>
          </div>
        </div>
    ...
    <div v-show="!addSubs">
      <button v-if="canEditTargetCtx" class="btn btn-primary" @click="addSubscriber()">
        <span class="fas fa-bolt" />
        <span>{{ $t('subscriber.home.createLabel') }}</span>
      </button>

      <div id="deleteTargetConfirmation" ref="deleteTargetConfirmation" class="modal fade">
        ...
        <table class="table table-striped">
          <thead>
            <tr>
              ...
              <th rowspan="2">
                {{ $t('subscriber.subject') }}
              </th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="target in targets" :key="getTargetId(target)">
              <td v-if="showSubscribeType()" :data-label="$t('subscriber.subscribeType')">
                {{ $t(getSubscribeTypeLabel(target.subscribeType)) }}
              </td>
              <td v-if="target.subjectDTO" :data-label="$t('subscriber.subject')">
                <esup-subject-infos
              ...
              <td v-if="!target.subjectDTO" :data-label="$t('subscriber.subject')">
                <esup-subject-infos .subject="target.subjectKeyExtendedDTO" .config="subjectInfosConfig" />
              </td>
              <td class="action">
                <button v-if="canEditTargetCtx" type="submit" class="btn btn-danger" @click="deleteSubscriber(target)">
                  <span class="far fa-trash-can" />&nbsp;<span>{{ $t('entity.action.delete') }}</span>
                </button>
              </td>
            </tr>
