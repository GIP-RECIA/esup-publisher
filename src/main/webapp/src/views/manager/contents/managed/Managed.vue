...
export default {
  ...
  computed: {
    ...
    sortedOrganizations() {
      const sortedOrganizations = Object.assign([], this.organizations)
  ...
  watch: {
    '$route.params.itemState': function () {
      this.loadAll()
    },
  },
  mounted() {
  ...
  methods: {
    ...
    getEnumKey(name) {
      const result = this.itemStateList.find(val => val.name === name)
      if (result) {
        return result.id
      }
    ...
    getEnumName(key) {
      const result = this.itemStateList.find(val => val.id === key)
      if (result) {
        return result.name
      }
      return 'PENDING'
...
<template>
  ...
  <div id="deleteItemConfirmation" ref="deleteItemConfirmation" class="modal fade">
  ...
  <div id="validateItemConfirmation" ref="invalidateItemConfirmation" class="modal fade">
    <div class="modal-dialog">
      <div class="modal-content">
        <form name="invalidateForm">
          <div class="modal-header">
            <h4 class="modal-title">
              {{ $t('entity.unvalidate.title') }}
            </h4>
            <button type="button" class="btn-close" aria-hidden="true" data-bs-dismiss="modal" />
          </div>
          <div class="modal-body">
          ...
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-danger" @click="confirmInvalidate(item.id)">
              <span class="far fa-check-circle" />&nbsp;<span>{{ $t('entity.action.unvalidate') }}</span>
            </button>
          </div>
        </form>
      </div>
  ...
  <div id="inValidateItemConfirmation" ref="validateItemConfirmation" class="modal fade">
    <div class="modal-dialog">
      <div class="modal-content">
        <form name="validateForm">
          <div class="modal-header">
            <h4 class="modal-title">
              {{ $t('entity.validate.title') }}
            </h4>
            <button type="button" class="btn-close" aria-hidden="true" data-bs-dismiss="modal" />
          </div>
          <div class="modal-body">
  ...
  <div id="tabs">
    ...
    <div class="table-responsive table-responsive-to-cards">
        ...
        <tbody>
          <tr v-for="item in items" :key="item.id" :class="{ highlight: item.highlight }">
            <td class="d-none" data-label="ID">
              <router-link :to="{ name: 'ContentDetail', params: { id: item.id } }">
                {{ item.id }}
              </router-link>
            </td>
            ...
            <td class="action">
              <button type="button" class="btn btn-info btn-sm me-1" @click="itemDetail(item)">
                <span class="far fa-eye" />&nbsp;<span>{{ $t('entity.action.view') }}</span>
              </button>
              <div v-can-edit="item.contextKey" class="d-inline">
                <button type="button" class="btn btn-primary btn-sm me-1" @click="update(item.id)">
                ...
                >
                  <span class="fas fa-ban" />&nbsp;<span>{{ $t('entity.action.unvalidate') }}</span>
                </button>
              </div>
              <button v-can-delete="item.contextKey" type="button" class="btn btn-danger btn-sm" @click="deleteItem(item.id)">
                <span class="far fa-trash-can" />&nbsp;<span>{{ $t('entity.action.delete') }}</span>
              </button>
            </td>
          </tr>
      ...
      <nav>
        <ul class="pagination">
          <li v-if="links.first" class="page-item" @click.prevent="loadPage(links.first)">
            <a class="page-link" href="">&lt;&lt;</a>
          </li>
          <li v-if="links.prev" class="page-item" @click.prevent="loadPage(links.prev)">
            <a class="page-link" href="">&lt;</a>
          </li>
          <li v-if="page > 2" class="page-item" @click.prevent="loadPage(page - 2)">
            <a class="page-link" href="">{{ page - 2 }}</a>
          </li>
          <li v-if="page > 1" class="page-item" @click.prevent="loadPage(page - 1)">
            <a class="page-link" href="">{{ page - 1 }}</a>
          </li>
          <li class="page-item active" @click.prevent="loadPage(page)">
            <a class="page-link" href="">{{ page }}</a>
          </li>
          <li v-if="page < links.last" class="page-item" @click.prevent="loadPage(page + 1)">
            <a class="page-link" href="">{{ page + 1 }}</a>
          </li>
          <li v-if="page < links.last - 1" class="page-item" @click.prevent="loadPage(page + 2)">
            <a class="page-link" href="">{{ page + 2 }}</a>
          </li>
          <li v-if="links.next" class="page-item" @click.prevent="loadPage(links.next)">
            <a class="page-link" href="">&gt;</a>
          </li>
          <li v-if="links.last" class="page-item" @click.prevent="loadPage(links.last)">
            <a class="page-link" href="">&gt;&gt;</a>
          </li>
        </ul>
      </nav>
