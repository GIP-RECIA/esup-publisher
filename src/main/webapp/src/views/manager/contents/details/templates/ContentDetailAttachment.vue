...
<template>
  <div>
    <h2>
      <span>{{ $t('attachment.detail.title') }}</span> ({{ $t(getItemTypeLabel(item.status)) }})
    </h2>
      ...
      <article>
        ...
        <section>
          <span v-for="attachment in filterLinkedFiles(linkedFiles)" :key="attachment.uri"><a :href="getUrlEnclosure(attachment.uri)" target="_blank" class="text-nowrap">
            <i :class="getCssFileFromType(attachment.contentType, attachment.filename)" />{{ attachment.filename }}</a>
          </span>
        </section>
      </article>
    </div>
