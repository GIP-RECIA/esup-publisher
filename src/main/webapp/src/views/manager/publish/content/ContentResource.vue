...
export default {
  ...
  watch: {
    ...
    'item.ressourceUrl': function (newVal) {
  ...
  methods: {
    ...
    onSelectFile() {
      ...
      if (file) {
        ...
        this.fileInputError = FormValidationUtils.getFileFieldError(file, 'image/*', ConfigurationService.getConfUploadFileSize(), false)
  ...
  <div>
    <div class="resourceForm">
      ...
      <div class="row">
        ...
        <div class="form-group col-md-4">
          ...
          <span v-if="publisher.context.redactor.optionalPublishTime" class="d-block d-md-none">{{
            $t('resource.detail.unlimitedEndDate')
          }}</span>
          <input
            id="endDate"
            v-model="itemEndDate"
      ...
      <div class="form-group">
        ...
        <input
          id="ressourceUrl"
          ref="ressourceUrlInput"
          v-model="itemRessourceUrl"
          type="url"
          class="form-control"
          name="ressourceUrl"
          placeholder="https://..."
          ...
          maxlength="2048"
        >
        <div v-if="formValidator.hasError('ressourceUrl', formErrors.REQUIRED)" class="invalid-feedback">
          {{ $t('entity.validation.required') }}
        </div>
        <div v-if="formValidator.hasError('ressourceUrl', formErrors.MAX_LENGTH)" class="invalid-feedback">
          {{ $t('entity.validation.maxlength', { max: '2048' }) }}
        </div>
        <div v-if="invalidRessourceUrl" class="invalid-feedback">
            ...
            <form name="publishContentEnclosureForm" role="form" novalidate>
              <div class="card">
                <h3 class="card-header">
                  {{ $t('resource.inputfile.url-label') }}
                </h3>
                <div class="card-body">
              ...
              <div class="card">
                <div class="card-header">
                  <h3 class="d-inline-block me-2">
                    {{ $t('resource.inputfile.upload-label') }}
                  </h3>
                  <button type="button" class="btn btn-primary btn-file" name="fileEnclosure" @click="fileInput.click()">
                    <i class="fas fa-folder-open" />&nbsp;<span>{{ $t('resource.inputfile.upload-button') }}</span>
                  </button>
                  <input id="file" ref="fileInput" type="file" name="file" accept="image/*" hidden @change="onSelectFile()">
                </div>
                <div class="card-body">
                  <div v-if="content.file" class="card">
                    <h4 class="card-header mt-0">
                      {{ $t('resource.inputfile.resizeauto') }}
                    </h4>
