...
export default {
  ...
  methods: {
    ...
    fileUploadedInBodyProgress(evt) {
      this.setProgressStatus('success')
      this.setProgress(Math.min(100, Number.parseInt((100.0 * evt.loaded) / evt.total)))
    },
  },
}
</script>
    ...
    <div class="newsForm">
      ...
      <div class="row">
        ...
        <div class="form-group col-md-4">
          ...
          <span v-if="publisher.context.redactor.optionalPublishTime" class="d-block d-md-none">{{
            $t('news.detail.unlimitedEndDate')
          }}</span>
          <input
            id="endDate"
            v-model="itemEndDate"
        ...
        <div class="form-group col-md-4 text-md-center">
          <label class="control-label" for="rssAllowed">{{ $t('news.rssAllowed') }}</label>
      ...
      <div class="form-group" :class="formValidator.hasError('body') ? 'invalid' : 'valid'">
        ...
        <RichText
          ...
          :errorImageSizeMsg="errorImageSizeMsg"
          :errorFileSizeMsg="errorFileSizeMsg"
          :callBackSuccess="fileUploadedInBodySuccess"
          :callBackError="fileUploadedInBodyError"
          :callBackProgress="fileUploadedInBodyProgress"
        />
        <div v-if="formValidator.hasError('body', formErrors.REQUIRED)" class="invalid-feedback d-block">
          {{ $t('entity.validation.required') }}
        </div>
        <div v-if="formValidator.hasError('body', formErrors.MIN_LENGTH)" class="invalid-feedback d-block">
      ...
      <div class="modal-dialog modal-fullscreen">
        <div class="modal-content">
              ...
              <div class="card">
                <div class="inputArea form-group">
                  ...
                  <button
                    ...
                    "
                  >
                    <i class="fas fa-folder-open" />&nbsp;<span>{{ $t('news.inputfile.upload-button') }}</span>
                  </button>
                  <input id="file" ref="fileInput" type="file" name="file" accept="image/*" hidden @change="selectFile()">
                  <p v-if="fileInputError === formErrors.PATTERN" class="help-block">
                    {{ $t('entity.validation.filepattern') }}
                  </p>
                  <p v-if="fileInputError === formErrors.MAX_SIZE" class="help-block">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-primary" :disabled="!content.file || !content.dataUrl" @click="uploadFile()">
            ...
            <button
              ...
              @click="clearUpload()"
            >
              <span class="fas fa-rotate-right" />&nbsp;<span>{{ $t('entity.action.reset') }}</span>
            </button>
