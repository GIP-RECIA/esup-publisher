...
export default {
  ...
  created() {
    ...
    if (this.publisher === null || this.publisher === undefined || !this.publisher.id) {
      ...
      if (this.itemTypeList.length === 1) {
        const oldVal = this.content.type
  ...
  methods: {
    initItem() {
      const entityID = this.publisher.context.organization.id
      const redactorID = this.publisher.context.redactor.id

      // tomorrow isn't more tomorrow as param should be 1 instead of 0
      const tomorrow = DateUtils.addDaysToLocalDate(this.today, 0)
      // warning should be < $scope.defaultMaxDuration
      const next4weeks = DateUtils.addDaysToLocalDate(this.today, 28)
    ...
    uploadLinkedFile(file, filename, isImage, isPublic, successCallBack) {
      const maxSize = isImage ? this.imageSizeMax : this.fileSizeMax
      if (file.size <= maxSize) {
        ...
        UploadUtils.upload(
          ...
          (evt) => {
            this.progress = Math.min(100, Number.parseInt((100.0 * evt.loaded) / evt.total))
          },
        )
        return true
      }
    ...
    removeEnclosure() {
      if (!CommonUtils.isDefined(this.item.enclosure) || this.item.enclosure == null)
        return
      this.deleteEnclosure()
    },
  },
...
<template>
  <div class="publish-content">
    ...
    <div v-if="itemTypeList.length > 1" class="form-group row">
      ...
      <div v-if="(item === null || item === undefined || !item.id) && itemTypeList.length > 1" class="col-md-8">
        <div v-for="itemType in itemTypeList" :key="itemType" class="form-check form-check-inline">
          <input
          ...
          >
          <label class="form-check-label fw-normal" :for="`itemType${itemType}`"><span>{{ $t(`enum.itemType.${itemType}`) }}</span></label>
        </div>
      </div>
    </div>
    ...
    <div class="text-center">
      <div class="btn-group" role="group">
        <router-link v-if="item === null || item.type !== 'FLASH'" v-slot="{ navigate }" to="classification" custom>
        ...
        <router-link v-if="item !== null && item.type === 'FLASH'" v-slot="{ navigate }" to="publisher" custom>
          <button type="button" class="btn btn-default btn-outline-dark btn-nav" @click="navigate">
            <span class="fas fa-arrow-left" />&nbsp;<span> {{ $t('entity.action.back') }}</span>
          </button>
        </router-link>
        ...
        <router-link v-if="goOnTargets()" v-slot="{ navigate }" to="targets" custom>
    ...
    <div id="deleteEnclosureConfirmation" ref="deleteEnclosureConfirmation" class="modal fade">
    ...
    <div id="deleteAttachmentConfirmation" ref="deleteAttachmentConfirmation" class="modal fade">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title">
