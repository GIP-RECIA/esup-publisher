...
export default {
  ...
  computed: {
    ...
    sortedOrganizations() {
      const sortedOrganizations = this.organizations
  ...
  methods: {
    ...
    filterFromReader(collection, search) {
      ...
      for (let i = 0, size = collection.length; i < size; i++) {
        if (collection[i].context.reader.id === Number.parseInt(search)) {
    ...
    filterFromRedactor(collection, search) {
      ...
      const filtered = []
      for (let i = 0, size = collection.length; i < size; i++) {
        if (collection[i].context.redactor.id === Number.parseInt(search)) {
          filtered.push(Object.assign({}, collection[i]))
        }
      }
      return filtered
...
<template>
  <div class="publish-publisher">
    ...
    <div v-if="publishers && publishers.length >= 1" class="form-group">
    ...
    <div v-if="selectedPublisher.context.reader && selectedPublisher.context.reader.id && redactors.length > 1" class="form-group">
    ...
    <div v-if="selectedPublisher.context.redactor && selectedPublisher.context.redactor.id" class="form-group">
    ...
    <div class="text-center">
      <div class="btn-group" role="group">
        ...
        <router-link v-slot="{ navigate }" to="classification" custom>
          <button
            type="button"
