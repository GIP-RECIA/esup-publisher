...
import CustomUploadAdapter from '@/components/richtext/CustomUploadAdapter.js'
...
vi.mock('compressorjs', () => ({ default: vi.fn((file, options) => options.success(new globalThis.Blob(['x'], { type: file.type }))) }))

describe('custom upload adapter', () => {
  const originalGetImageSize = ConfigurationService.getConfUploadImageSize

  beforeEach(() => {
    vi.clearAllMocks()
    vi.stubEnv('VITE_BACK_BASE_URL', 'https://publisher.example/')
    ConfigurationService.getConfUploadImageSize = vi.fn().mockReturnValue(1000000)
  })
  ...
  it('uploads a file and returns the decoded backend URL', async () => {
    ...
    const adapter = new CustomUploadAdapter({ file: Promise.resolve(file) }, 42, 1000, 'too large', successCallback)

    await expect(adapter.upload()).resolves.toEqual({ default: 'https://publisher.example/view/file/document.pdf' })
    expect(UploadUtils.upload).toHaveBeenCalledWith('app/upload/', expect.objectContaining({ file, entityId: 42, isPublic: false, name: 'document.pdf' }), expect.any(Function), expect.any(Function), expect.any(Function))
  ...
  it('rejects an oversized file and does not upload it', async () => {
    ...
    const adapter = new CustomUploadAdapter({ file: Promise.resolve(file) }, 42, 1, 'too large', null, errorCallback)
  ...
  it('compresses an oversized image before uploading', async () => {
    ...
    adapter.compressImage = vi.fn().mockResolvedValue(compressedFile)
    UploadUtils.upload.mockImplementation((url, data, success) => {
